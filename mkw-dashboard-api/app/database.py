...
class DatabaseManager:
    ...
    def get_connection(self):
        ...
        try:
            ...
            logger.error(f"❌ Dashboard API database connection timeout or network error: {e}")
            if conn:
                conn.rollback()
            raise
        except psycopg2.errors.QueryCanceled as e:
            # Statement timeout - query exceeded 30 seconds
            logger.error(f"❌ Dashboard API database query timeout (exceeded 30s statement_timeout): {e}")
            if conn:
                conn.rollback()
            raise
        except Exception as e:
